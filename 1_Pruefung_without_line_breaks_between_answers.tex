...
\begin{choices}
\CorrectChoice Betrachtet man die Grafik, lässt sich der Zusammenhang gut durch eine Polynomfunktion
...
\CorrectChoice Sähe man nur die Daten im Bereich von $x=-3$ bis $x=0$, wäre der Korr
\end{choices}
...
\begin{choices}
\CorrectChoice Wir dürfen annehmen, das Quantil der $\chi^2$-Verteilun
...
\CorrectChoice Der $p$-Wert gibt die Wahrscheinlichkeit an, unter der Annahme, dass di
\end{choices}
...
\begin{choices}
\choice Da die Ereignisse nicht disjunkt (disjoint) sind, können sie auch nicht u
\choice Die Wahrscheinlichkeit der Vereinigung $P(A \cup B)$ ist gleich groß oder größer als die
...
\begin{choices}
\CorrectChoice Die Wahrscheinlichkeit, dass der Test positiv ist, wenn die Person di
\choice Die Wahrscheinlichkeit, dass die Person die Krankheit hat, wenn der T
\CorrectChoice Die Wahrscheinlichkeit, dass der Test positiv ist, wenn die P
...
\begin{choices}
\CorrectChoice Falls $\alpha=0.05$, $\beta=0.2$ und $P=0.5$, dann ist die Wahrscheinlichkeit dafür eine falsche
\CorrectChoice Ein $\beta$-Fehler oder \textit{Fehler 2. Art} ist die Wahrscheinlichkeit, unter der Annahme, dass $H_
\CorrectChoice Ein $\alpha$-Fehler oder \textit{Fehler 1. Art}, ist die Wahrscheinlichkeit, unter der Anna
...
\choice Der Shapiro-Wilk-Test zeigt einen $p$-Wert von $0.5761$. Wir können daher s
\CorrectChoice Der QQ-Plot ist mit einer Normalverteilung vereinbar.
\CorrectChoice Der QQ-Plot könnte auch von einer $t$-Verteilung mit höherer Anza
\end{choices}
...
\choice Mit 95\%iger Wahrscheinlichkeit liegt die Mittelwertdiffere
\CorrectChoice Die Testentscheidung lautet: Die Mittelwertdifferenz ist wahrsch
\end{choices}
...
\\
\begin{choices}
\CorrectChoice Der $p$-Wert für diesen zweiseitigen Test beträgt (
...
\CorrectChoice Der Erwartungswert ist der Massenschwerpunkt über der Dich
\CorrectChoice Für eine diskrete Zufallsvariable $X$ ist der Erwartungswer
\end{choices}
...
\\
\begin{choices}
\choice $p(x)$ stellt das Vorwissen dar, das man über den interessierenden
\choice $p(x | \theta)$ nennt man auch \textit{posterior distribution} des interessiere
\CorrectChoice $p(x)$ ist im Allgemeinen schwer zu berechnen, weshalb man Monte
\CorrectChoice In der Bayes-Praxis erspart man sich die Bestimmung der Verteilung einer T
\end{choices}
...
\begin{choices}
\choice Da das QQ-Plot darauf hinweist, dass die Daten nicht normalverteilt sind, kann
...
\\
\begin{choices}
\CorrectChoice Für ein konkretes (klassisches) Konfidenzinterval gilt: Der wahre
\choice Sei $[a,b]$ ein 95\%-Konfidenzintervall für einen unbek
\CorrectChoice Sei $[a,b]$ ein 95\% \textit{Credible interval}, das
\CorrectChoice Wenn man viele (klassischken ca. 95\% der Inte
\end{choices}
...
\choice Der wahre aber unbekannte Parameter ($\rho$) liegt mit 95\%iger 
\choice $H_0: \rho \ge 0$.
\end{choices}
...
\begin{choices}
\CorrectChoice Man ist sich a priori nicht ganz sicher,
\CorrectChoice Werte um $\theta = 0.3$ und $\theta=0.7Werte um $\theta=0.5$.
\CorrectChoice Würde man in einer neuen Studie einen Therapieeffekt vo
\CorrectChoice Die a priori Verteilung 
\end{choices}
...
\choice Hier bekommt man kein "signif
\choice Angenommen, es gäbe einen Gruppenmitte
...
\CorrectChoice Bei starkem Vorwissen (gute Studie Prior Verteilung maßgeblich zu verändern.
\CorrectChoice Die Reihenfolge der Daten spielt bei der Bayes-Statistik keine Rolle (Bayes Updating).
\CorrectChoice Im Gegensatz zur frequentistischen Statistiik eine Wfür den interessierenden Parameter.  
\end{choices}
...
\begin{choices}
\CorrectChoice Wenn die \(X_i\) Gamma-verteilt sind (und die restlichen Vor
\CorrectChoice Wenn die \(X_i\) Binomial-verteilt sind (und die restlichen Voraussetzungen für das T
\choice Die Verteilung von \(\frac{\bar{X} - \mu}{\sigma / \sqrt{n
...
\end{figure}
\begin{choices}
\choice Es handelt sich um einen einseitigecht-negative Werte annimmt.
\CorrectChoice Die beobachteten Unterschidurch Zufall alleine gut erklärbar. 
\CorrectChoice Drei Freiheitsgrade hat man, da man bei bek
...
\begin{choices}
\choice Bootstrap-Konfidenzintervalle basieren auf der $t$-alverteilungsannahmen.
\choice Die Breite eines Bootstrap-Konfidenzinten) Stichprobengröße ab.
\CorrectChoice Bootstrap-Konfidenzintervalle werdet Zus den Originaldaten erstellt.
\choice Das Bootstrap-Verfahren liefert exakte Konfidenzinte von der Verteilung der Daten.
\end{choices}
